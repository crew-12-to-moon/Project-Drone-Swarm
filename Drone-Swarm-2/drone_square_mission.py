...
class DroneController(Node):
    ...
    def execute_mission(self):
        ...
        if self.takeoff(2.0):
            ...
            # Define the square path offsets
            square_path = [
                (0.5, 0.0),  # Move 0.5m in x direction
                (0.5, 0.5),  # Move 0.5m in y direction
                (0.0, 0.5),  # Move -0.5m in x direction
                (0.0, 0.0)   # Move -0.5m in y direction (back to start)
            ]
