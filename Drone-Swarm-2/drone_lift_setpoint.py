...
class DroneController(Node):
    def __init__(self):
        ...
        # Added publisher for setpoint local position (PoseStamped)
        self.position_publisher = self.create_publisher(PoseStamped, '/mavros/setpoint_position/local', qos_profile)
    ...
    # New helper function to publish setpoint positions via the local topic
    def publish_position(self, target_altitude):
        msg = PoseStamped()
        msg.header.stamp = self.get_clock().now().to_msg()
        # Set a fixed position (x=0, y=0) with the desired altitude.
    ...
    def execute_mission(self):
        if self.takeoff(1.0):
            ...
            while self.current_altitude < 2.0:
                ...
                # Continue to command using velocity setpoints
                self.publish_velocity(0.2)
                # Also publish a setpoint position for altitude 2m
            ...
            while self.current_altitude > 0.2:
                ...
                # Continue to command using velocity setpoints
                self.publish_velocity(-0.2)
                # Also publish a setpoint position for ground level (0m)
